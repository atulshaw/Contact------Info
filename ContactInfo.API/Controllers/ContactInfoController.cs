...
namespace ContactInfo.API.Controllers
{
    ...
    public class ContactInfoController : ApiController
    {
        ...
        [HttpPost]
        [Route("DeleteContact")]
        public async Task<IHttpActionResult> DeleteContact(Deleteparam conID)
        {
            try
            {
                var result = _contactInfoService.DeleteContactInfo(Convert.ToInt32(conID.DelContactID));
                return await Task.FromResult(Json(new { Result = result }));
            }
            catch (Exception ex)
            {
